...
import { TLatestOrder, TLatestUser, TMonthlyUserGrowth } from "@/types/dashboard.type";


const COLORS = ["#00A300", "#facc15", "#ef4444", "#7ec8e3"];

const AdminDashboard = () => {
    const { data: dashboardData, isLoading } = useGetAdminDashboardDataQuery(undefined)
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <Card>
                    <CardContent className="p-4">
                        ...
                        <p className="text-3xl font-bold text-red-600">{dashboardInfo?.totalUsers}</p>
                    </CardContent>
                </Card>
                <Card>
                    <CardContent className="p-4">
                        <p className="text-lg font-semibold">Total Products</p>
                        <p className="text-3xl font-bold text-red-600">{dashboardInfo?.totalCars}</p>
                    </CardContent>
                </Card>
                <Card>
                    <CardContent className="p-4">
                        <p className="text-lg font-semibold">Pending Orders</p>
                        <p className="text-3xl font-bold text-red-600">{dashboardInfo?.totalPendingOrders}</p>
                    </CardContent>
                </Card>
                <Card>
                    <CardContent className="p-4">
                        <p className="text-lg font-semibold">Revenue This Month</p>
                        <p className="text-3xl font-bold text-red-600">${dashboardInfo?.totalSales}</p>
                    </CardContent>
                </Card>
            </div>
                ...
                <Card>
                    <CardContent className="p-4">
                        ...
                        <ResponsiveContainer width="100%" height={200}>
                            <BarChart data={dashboardInfo?.monthlyUserGrowth as TMonthlyUserGrowth[]}>
                                <XAxis dataKey="month" />
                                <YAxis />
                                <Tooltip />
                ...
                <Card>
                    <CardContent className="p-4">
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {
                                    dashboardInfo?.last8Users?.map((user: TLatestUser) => <TableRow key={user?._id}>
                ...
                <Card>
                    <CardContent className="p-4">
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {
                                    dashboardInfo?.last8Orders.map((order: TLatestOrder) => <TableRow key={order?._id}>
